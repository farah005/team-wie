...
div[data-testid="stButton"] button:hover {
  background: #b50b0c !important; box-shadow: 0 0 20px rgba(230,14,15,.4) !important;
  transform: translateY(-1px) !important;
}
.stSelectbox > div > div { background:#1e1e1e !important; border-color:#3a3a3a !important; color:white !important; }
...
[data-testid="stDataFrame"] { background:#141414 !important; }
...
if not st.session_state.logged_in:
    ...
    with center:
        ...
        st.markdown('<div style="margin-bottom:8px;font-size:12px;color:#888;text-transform:uppercase;letter-spacing:.08em">Votre profil</div>', unsafe_allow_html=True)
        persona_choice = st.radio("", list(PERSONAS.keys()), label_visibility="collapsed")
...
with st.sidebar:
    ...
    PERSONA_ICONS = {
        "Entreprise":"🏢","Journalisme":"📰","Influenceur":"🎬",
        "Sport":"⚽","Mode":"👗","Grande Surface":"🏪","Citoyen":"👤"
    }
    st.markdown(f"""
    <div style="background:#1e1e1e;border:1px solid #3a3a3a;border-radius:10px;padding:10px 14px;margin-bottom:16px">
      <div style="font-size:11px;color:#888;text-transform:uppercase;letter-spacing:.06em">Profil actif</div>
    ...
    st.markdown('<div style="font-size:11px;color:#666;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px">Filtres</div>', unsafe_allow_html=True)

    REGIONS_LIST = ["Toutes"] + sorted(REGION_COORDS.keys())
    PLATFORMS    = ["Toutes","Facebook","TikTok","Instagram","RSS","Google Trends"]
    ...
    st.markdown(f"""
    <div style="margin-top:20px;padding:10px;background:#0d0d0d;border-radius:8px;font-size:10px;color:#555;text-align:center">
      <span class="pulse-dot"></span>Live · TNessnisa 2026
    </div>
    """, unsafe_allow_html=True)

# ─── Charger données ───────────────────────────────────────────────────────────
@st.cache_data(ttl=30)
def load_data(region, platform):
    posts = get_posts(200, region if region!="Toutes" else None, platform if platform!="Toutes" else None)
    dash  = get_persona_dashboard(persona, posts)
    return posts, dash

posts, dash = load_data(region_filter, platform_filter)
stats = dash["stats"]
...
with col_h1:
    ...
    <div style="padding: 8px 0 16px">
      ...
      <div style="font-size:13px;color:#888;margin-top:3px">{PERSONA_DESCS.get(persona,'Intelligence sociale personnalisée')}</div>
    </div>
    """, unsafe_allow_html=True)
with col_h2:
    st.markdown(f"""
    <div style="text-align:right;padding-top:8px">
      <span style="background:#1e1e1e;border:1px solid #3a3a3a;border-radius:8px;padding:6px 12px;font-size:12px;color:#888">
...
if persona == "Entreprise":
    ...
    with tab1:
        ...
        # Scatter map simulée avec Plotly
        lats, lons, labels, colors, sizes, texts = [], [], [], [], [], []
        # Approximation lat/lon depuis pourcentages (Tunisie: lat 30-37, lon 8-11.5)
        for reg, info in region_data.items():
            ...
            labels.append(reg); colors.append(info["color"])
            sz = max(12, min(40, 12 + info["score"]//4))
            sizes.append(sz)
            txt = f"<b>{reg}</b><br>{info['label']}<br>Score: {info['score']}<br>Posts: {info['count']}"
            texts.append(txt)
        ...
        fig_map.add_trace(go.Scattergeo(
            ...
            mode="markers+text",
            texttext=labels, textposition="top center",
            textfont=dict(size=9, color="white")
        ))
        fig_map.update_layout(
            geo=dict(scope="africa", center=dict(lat=33.8,lon=9.5), projection_scale=8,
                     showland=True, landcolor="#1a1a1a", showocean=True, oceancolor="#0d0d0d",
                     showcoastlines=True, coastlinecolor="#3a3a3a",
        ...
        for col, (color, label) in zip(leg_cols, [("#e60e0f","🔴 Crise / Accident"),("#22c55e","🟢 Opportunité"),("#f59e0b","🟡 Sensible / Triste"),("#3a3a3a","⚪ Neutre")]):
            col.markdown(f'<div style="display:flex;align-items:center;gap:6px;font-size:12px;color:#ccc"><div style="width:10px;height:10px;border-radius:50%;background:{color}"></div>{label}</div>', unsafe_allow_html=True)

        # Détail par région
        st.markdown('<div class="section-title">Détail par région</div>', unsafe_allow_html=True)
        rcols = st.columns(3)
        for idx, (reg, info) in enumerate([r for r in region_data.items() if r[1]["count"]>0]):
            with rcols[idx % 3]:
                ...
                <div class="tn-card">
                  ...
                  <div class="gauge-wrap">
                    ...
                    <div class="gauge-track"><div class="gauge-fill" style="width:{info['score']}%;background:{info['color']}"></div></div>
                  </div>
    ...
    with tab2:
        ...
        if not crises:
            ...
            st.markdown(f'<div style="background:rgba(230,14,15,.1);border:1px solid rgba(230,14,15,.3);border-radius:10px;padding:12px 16px;margin-bottom:16px;font-size:13px;color:#ff6666">⚠️ {len(crises)} situation(s) critique(s) détectée(s) — Recommandation : <b>Pause publicitaire</b> dans les zones concernées</div>', unsafe_allow_html=True)
            for p in crises:
                vs = int(p.get("viral_score",0)); fs = int(p.get("fake_score",0))
                st.markdown(f"""
                <div class="tn-card">
                  <div style="display:flex;justify-content:space-between;align-items:flex-start">
                    <div style="flex:1">
                      <div style="font-size:13px;font-weight:600;color:#fff;margin-bottom:6px">{p.get('content','')[:100]}…</div>
                      <div>
                    ...
                    <div style="text-align:right;min-width:90px">
                      <div style="font-size:24px;font-weight:800;color:#e60e0f">{vs}</div>
                      <div style="font-size:10px;color:#666">viral score</div>
                      <span class="action-btn action-pause">STOP PUB</span>
                    </div>
                  </div>
                  <div class="gauge-wrap" style="margin-top:10px">
                    <div class="gauge-label"><span>Viralité</span><span>{vs}%</span></div>
                    <div class="gauge-track"><div class="gauge-fill" style="width:{vs}%;background:#e60e0f"></div></div>
                  </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            for p in oppos:
                ...
                <div class="tn-card green">
                  <div style="display:flex;justify-content:space-between;align-items:flex-start">
                    ...
                    <div style="text-align:right;min-width:90px">
                      <div style="font-size:24px;font-weight:800;color:#22c55e">{vs}</div>
                      <div style="font-size:10px;color:#666">viral score</div>
                  ...
                  <div class="gauge-wrap" style="margin-top:10px">
                    <div class="gauge-label"><span>Viralité</span><span>{vs}%</span></div>
                    <div class="gauge-track"><div class="gauge-fill" style="width:{vs}%;background:#22c55e"></div></div>
                  </div>
                </div>
                """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        if not urgent:
            ...
            for i, p in enumerate(urgent):
                vs = int(p.get("viral_score",0)); fs = int(p.get("fake_score",0))
                fake_badge = f'<span class="badge badge-yellow">⚠️ Fake {fs}%</span>' if fs>35 else f'<span class="badge badge-green">✅ Vérifié {100-fs}%</span>'
                st.markdown(f"""
                <div class="tn-card" style="border-left-color:{'#e60e0f' if i==0 else '#888'}">
                  <div style="display:flex;gap:12px;align-items:flex-start">
                    <div style="font-size:22px;font-weight:900;color:{'#e60e0f' if i<3 else '#555'};min-width:28px">#{i+1}</div>
                    <div style="flex:1">
                      <div style="font-size:14px;font-weight:600;color:#fff;margin-bottom:6px;line-height:1.4">{p.get('content','')}</div>
                      <div>
                        <span class="badge badge-red">📍 {p.get('region','')}</span>
                        <span class="badge badge-gray">{p.get('source','')}</span>
                        <span class="badge badge-gray">{p.get('platform','')}</span>
                        {fake_badge}
                        <span class="badge badge-blue">{EMOTION_EMOJI.get(p.get('emotion',''),'')}{p.get('emotion','')}</span>
                      </div>
                    </div>
                    <div style="text-align:center;min-width:60px">
                      <div style="font-size:26px;font-weight:900;color:#e60e0f">{vs}</div>
                      <div style="font-size:9px;color:#555;text-transform:uppercase">viral</div>
                    </div>
                  </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if not fake_sus:
            ...
            st.markdown(f'<div style="background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.3);border-radius:10px;padding:12px 16px;margin-bottom:16px;font-size:13px;color:#f59e0b">⚠️ {len(fake_sus)} publication(s) avec score de fiabilité faible</div>', unsafe_allow_html=True)
            for p in fake_sus:
                fs = int(p.get("fake_score",0))
                reliability = 100 - fs
                st.markdown(f"""
                <div class="tn-card yellow">
                  <div style="font-size:13px;font-weight:600;color:#fff;margin-bottom:8px">{p.get('content','')[:100]}…</div>
                  <div style="margin-bottom:8px">
                    <span class="badge badge-yellow">⚠️ Fiabilité {reliability}%</span>
                    <span class="badge badge-gray">{p.get('source','')}</span>
                    <span class="badge badge-gray">📍 {p.get('region','')}</span>
                  </div>
                  <div class="gauge-wrap">
                    <div class="gauge-label"><span>Score de risque</span><span>{fs}%</span></div>
                    <div class="gauge-track"><div class="gauge-fill" style="width:{fs}%;background:#f59e0b"></div></div>
                  </div>
                  <div style="font-size:11px;color:#888;margin-top:6px">Recommandation : Vérifier via 3+ sources officielles avant publication</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with col_g1:
            ...
            if "region" in df.columns:
                ...
                fig = px.bar(reg_counts, x="count", y="region", orientation="h",
                             color="count", color_continuous_scale=["#1e1e1e","#e60e0f"],
                             title="Activité par région")
                fig.update_layout(paper_bgcolor="#000",plot_bgcolor="#000",font_color="#ccc",
                                  title_font_color="#fff",coloraxis_showscale=False,
                                  margin=dict(l=0,r=0,t=40,b=0),height=300)
                fig.update_xaxes(gridcolor="#2a2a2a"); fig.update_yaxes(gridcolor="#2a2a2a")
                st.plotly_chart(fig, use_container_width=True)
        with col_g2:
            if "emotion" in df.columns:
                em_counts = df["emotion"].value_counts().reset_index()
                em_counts.columns = ["emotion","count"]
                COLORS_EM = {"Panique":"#e60e0f","Colère":"#f97316","Hype":"#22c55e",
                             "Tristesse":"#3b82f6","Humour":"#f59e0b","Neutre":"#555","Solidarité":"#ec4899"}
                colors_list = [COLORS_EM.get(e,"#888") for e in em_counts["emotion"]]
                fig2 = px.pie(em_counts, values="count", names="emotion", title="Distribution émotions",
                              color_discrete_sequence=colors_list, hole=0.55)
                fig2.update_layout(paper_bgcolor="#000",font_color="#ccc",title_font_color="#fff",
                                   margin=dict(l=0,r=0,t=40,b=0),height=300,
                                   legend=dict(bgcolor="#000",font=dict(color="#ccc")))
                st.plotly_chart(fig2, use_container_width=True)

# ══════════════════════════════════════════════════════════════════════════════
# PERSONA : INFLUENCEUR / SPORT / MODE / GRANDE SURFACE
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        if not trends:
            ...
            for i, p in enumerate(trends):
                vs = int(p.get("viral_score",0))
                timing = "20h–22h" if vs>85 else "18h–20h" if vs>70 else "Tester"
                st.markdown(f"""
                <div class="tn-card green">
                  <div style="display:flex;gap:12px;align-items:center">
                    <div style="font-size:28px;font-weight:900;color:#22c55e;min-width:36px">#{i+1}</div>
                    <div style="flex:1">
                      <div style="font-size:13px;font-weight:600;color:#fff;margin-bottom:6px">{p.get('content','')[:90]}…</div>
                      <div>
                        <span class="badge badge-green">📍 {p.get('region','')}</span>
                        <span class="badge badge-gray">{p.get('platform','')}</span>
                        <span class="badge badge-green">🔥 Viral</span>
                        <span class="badge badge-blue">⏰ {timing}</span>
                      </div>
                    </div>
                    <div style="text-align:center;min-width:70px">
                      <div style="font-size:30px;font-weight:900;color:#22c55e">{vs}</div>
                      <div style="font-size:9px;color:#555;text-transform:uppercase">score</div>
                      <span class="action-btn action-go" style="font-size:10px;padding:3px 10px">UTILISER</span>
                    </div>
                  </div>
                  <div class="gauge-wrap" style="margin-top:8px">
                    <div class="gauge-track"><div class="gauge-fill" style="width:{vs}%;background:linear-gradient(90deg,#22c55e,#4ade80)"></div></div>
                  </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            for p in avoid:
                ...
                <div class="tn-card">
                  <div style="display:flex;gap:12px;align-items:center">
                    ...
                    <span class="action-btn action-pause">ÉVITER</span>
                  </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        with col_g1:
            if "platform" in df.columns:
                ...
                fig = px.bar(plt_counts, x="platform", y="count", title="Posts par plateforme",
                             color="count", color_continuous_scale=["#1e1e1e","#e60e0f"])
                fig.update_layout(paper_bgcolor="#000",plot_bgcolor="#000",font_color="#ccc",
                                  title_font_color="#fff",coloraxis_showscale=False,
        ...
        with col_g2:
            if "viral_score" in df.columns:
                ...
                fig2 = px.bar(df_vs, x="viral_score", y="region", orientation="h",
                              title="Top viral par région", color="viral_score",
                              color_continuous_scale=["#22c55e","#e60e0f"])
                fig2.update_layout(paper_bgcolor="#000",plot_bgcolor="#000",font_color="#ccc",
                                   title_font_color="#fff",coloraxis_showscale=False,
                                   margin=dict(l=0,r=0,t=40,b=0),height=280)
                fig2.update_xaxes(gridcolor="#2a2a2a"); fig2.update_yaxes(gridcolor="#2a2a2a")
                st.plotly_chart(fig2, use_container_width=True)
            ...
            for p in alerts:
                ...
                <div class="tn-card">
                  <div style="font-size:13px;font-weight:600;color:#fff;margin-bottom:6px">
                    {EMOTION_EMOJI.get(p.get('emotion',''),'🔔')} {p.get('content','')[:100]}…
                  </div>
                  <div>
                    <span class="badge badge-red">📍 {p.get('region','')}</span>
                    <span class="badge badge-gray">{p.get('source','')}</span>
                    {'<span class="badge badge-yellow">⚠️ Vérifier</span>' if p.get("fake_score",0)>40 else ''}
                  </div>
                  <div style="font-size:11px;color:#888;margin-top:6px">Restez vigilant — information en cours de vérification</div>
                </div>
                """, unsafe_allow_html=True)
    with tab2:
        local = dash["local"]
        for p in local:
            ...
            <div class="tn-card blue">
              <div style="font-size:13px;color:#fff;margin-bottom:6px">{p.get('content','')[:100]}…</div>
              <div>
                <span class="badge badge-blue">📍 {p.get('region','')}</span>
                <span class="badge badge-gray">{p.get('platform','')}</span>
                {f'<span class="badge badge-green">🔥 {vs}</span>' if vs>0 else ''}
              </div>
            </div>
            """, unsafe_allow_html=True)
...
with st.expander("📋  Tableau complet des données"):
    df_show = pd.DataFrame(posts)[["region","platform","source","content","emotion","viral_score","fake_score","timestamp"]].copy() if posts else pd.DataFrame()
    if not df_show.empty:
        df_show.columns = ["Région","Plateforme","Source","Contenu","Émotion","Viral","Fake%","Date"]
        df_show["Contenu"] = df_show["Contenu"].str[:60]+"…"
